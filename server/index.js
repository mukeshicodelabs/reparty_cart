...
const { dropBoxWebhook } = require('./api/dropBoxSign');
...
app.post('/rental-agg-webhook', upload.none(), dropBoxWebhook);
...
if (!dev) {
  ...
  if (hasUsername && hasPassword) {
        // Create a middleware that skips basic auth for webhooks and third-party endpoints
    app.use((req, res, next) => {
      // Apply basic auth for all other routes
      return auth.basicAuth(USERNAME, PASSWORD)(req, res, next);
    });
  }
}
