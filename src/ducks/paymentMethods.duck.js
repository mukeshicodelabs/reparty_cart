...
import { getShippingRates, validatingAddressWithShippo } from '../util/api';
...
export const CHECK_DELIVERY_LOCATION_REQUEST = 'app/paymentMethods/CHECK_DELIVERY_LOCATION_REQUEST';
export const CHECK_DELIVERY_LOCATION_SUCCESS = 'app/paymentMethods/CHECK_DELIVERY_LOCATION_SUCCESS';
export const CHECK_DELIVERY_LOCATION_ERROR = 'app/paymentMethods/CHECK_DELIVERY_LOCATION_ERROR';
...
const initialState = {
  ...
  stripeCustomer: null,
  checkDeliveryLocationInProgress: false,
  checkDeliveryLocationError: null,
  deliveryLocationResponse: null,
};
...
export default function payoutMethodsPageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case DELETE_PAYMENT_METHOD_ERROR:
      ...
      case CHECK_DELIVERY_LOCATION_REQUEST:
        return { ...state, checkDeliveryLocationError: null, checkDeliveryLocationInProgress: true };
      case CHECK_DELIVERY_LOCATION_SUCCESS:
        return {
          ...state,
          checkDeliveryLocationInProgress: false,
          deliveryLocationResponse: payload,
        };
      case CHECK_DELIVERY_LOCATION_ERROR:
        console.error(payload);
        return {
          ...state,
          checkDeliveryLocationError: payload,
          checkDeliveryLocationInProgress: false,
        };
    default:
      return state;
...
export const checkDeliveryLocationRequest = () => ({
  type: CHECK_DELIVERY_LOCATION_REQUEST,
});

export const checkDeliveryLocationSuccess = response => ({
  type: CHECK_DELIVERY_LOCATION_SUCCESS,
  payload: response,
});

export const checkDeliveryLocationError = e => ({
  type: CHECK_DELIVERY_LOCATION_ERROR,
  payload: e,
  error: true,
});
...
export const checkDeliveryLocation = params => async (dispatch, getState, sdk) => {
  dispatch(checkDeliveryLocationRequest());
  try {
    const response = await validatingAddressWithShippo({ params });
    dispatch(checkDeliveryLocationSuccess(response));
    const { is_valid } = response.validation_results || false;
 
        if (!is_valid) { 
          dispatch(checkDeliveryLocationError("Not a valid Address"))
        }  

    return response;
  } catch (e) {
    log.error(storableError(e), 'check-delivery-location-failed');
    // dispatch(checkDeliveryLocationError(storableError(e)));
    throw e;
  }
};
export const fetchPriceFromShipo = (params) => async(
  dispatch,
  getState,
  sdk
) => {  
  const getrespo = await getShippingRates({params})
  return getrespo; 
};
