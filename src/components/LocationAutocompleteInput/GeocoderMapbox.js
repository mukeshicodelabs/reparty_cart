  ...
  getPlacePredictions(search, countryLimit, locale) {
    ...
    return this.getClient()
      .geocoding.forwardGeocode({
        ...
        limit: 5,
        ...limitCountriesMaybe,
        types: ['postcode', 'address', 'place'], // Include postcode search
        language: [locale],
      })
      .send()
      .then(response => {
  ...
  getPlaceDetails(prediction, currentLocationBoundsDistance) {
    ...
    const context = prediction?.context || [];
     ...
     const postcode = context.find(c => c.id.startsWith('postcode'))?.text || '';
      ...
      // Street is not in context — it's in feature.address + feature.text
      const street = prediction.address
        ? `${prediction.address} ${prediction.text}`
        : prediction.text || '';
    ...
    return Promise.resolve({
      ...
      fullAddress: prediction.place_name,
      street,
      postcode,
      city,
      state,
