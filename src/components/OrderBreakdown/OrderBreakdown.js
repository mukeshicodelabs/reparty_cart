...
export const OrderBreakdownComponent = props => {
  ...
  const { securityDepositAmount } = transaction?.attributes?.protectedData || {}
  const isSecuityDeposit = transaction?.attributes?.lastTransition == "transition/deposit-security-amount";
  const secuityDepositCode = "line-item/security-deposit";

  if (isSecuityDeposit && securityDepositAmount && allLineItems.findIndex(st => st.code == secuityDepositCode) < 0) {
    const secuityDepositPrice = new Money(securityDepositAmount * 100, currency);
    const securityLineItem = {
      code: secuityDepositCode,
      lineTotal: secuityDepositPrice,
      unitPrice: secuityDepositPrice,
      reversal: false,
      quantity: new Decimal(1),
      includeFor: ['customer', 'provider']
    }
    allLineItems.push(securityLineItem);
  };
  ...
  return (
    <div className={classes}>
      ...
      <LineItemBasePriceMaybe lineItems={lineItems} code={lineItemUnitType} intl={intl} breakdownData={breakdownData} />
