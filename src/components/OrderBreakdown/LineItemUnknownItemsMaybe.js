  ...
  return items.length > 0 ? (
    <React.Fragment>
      {items.map((item, i) => {
        ...
        const quantity = item?.units ? item.units.toString() : item?.quantity;
        const unitPrice = item?.unitPrice ? formatMoney(intl, item.unitPrice) : null;
        const label = item?.seats ? ( 
          <>
            Item {" " + (i+ 1) + " - "}
           <FormattedMessage
            id={`${'OrderBreakdown.baseUnitDay'}Seats`}
            values={{ unitPrice, quantity, seats: item.seats }}
          />
          </> 
        ) : quantity && quantity > 1 ? (
