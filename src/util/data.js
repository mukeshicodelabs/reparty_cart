...
// Helper function to validate availability dates
export const getValidAvailabilityDates = (availability) => {
  return availability
    ?.map(item => {
      const start = new Date(item?.data?.[0]?.attributes?.start);
      const end = new Date(item?.data?.[0]?.attributes?.end);
      return { start, end };
    })
    .filter(({ start, end }) => !isNaN(start) && !isNaN(end)); // Filter out invalid dates
};
// Helper function to check if endDate is within availability
export const isDateAvailable = (endDate, availabilityDates) => {
  const itemEndDate = new Date(endDate);
  return availabilityDates.some(({ start, end }) => itemEndDate >= start && itemEndDate <= end);
};
export const isProductForRent = (productType) => {
  return (
      productType === 'rent'
  );
}

export const deliveryPickupMethodOptions = [
  {
    key: 'delivery',
    label: 'Shipping',
  },
...
export const SELECT_DELIVERY = 'seleteDelivery';
export const ADD_DELIVERY_LOCATION = 'addDeliveryLocation';
export const SELECT_PACKAGE = 'selectPackage';
