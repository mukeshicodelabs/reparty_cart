...
export const processCheckoutWithPayment = (orderParams, extraPaymentParams) => {
  ...
  // const processAlias =  cartItems?.length ? "default-purchase-cart/release-1" : pageData?.listing?.attributes?.publicData?.transactionProcessAlias;
  const processAlias =
    Array.isArray(cartItems) && cartItems?.length && !cartItems?.[0]?.isRental
      ? 'default-purchase-cart/release-1'
  ...
  const fnConfirmPayment = fnParams => {
    ...
    const listingType = fnParams?.listing?.productType == "rent"; 
    const isNeedFormutliTransactionCheckout =  Array.isArray(pageData?.orderData?.otherOrderData?.cartItems) && pageData?.orderData?.otherOrderData?.cartItems?.length > 1 && !pageData?.orderData?.otherOrderData?.cartItems?.[0]?.isRental;
    ...
    const orderPromise = isTransitionedAlready
      ? Promise.resolve(storedTx)
      : isNeedFormutliTransactionCheckout
      ? onConfirmPaymentMultiTransactionCheckout({
          transactionId,
      ...
      : onConfirmPayment(transactionId, transitionName, {
          protectedData: {
            sharetribePaymentIntent: createdPaymentIntent.id,
          },
        });  
    orderPromise.then(order => {
      // Store the returned transaction (order)
