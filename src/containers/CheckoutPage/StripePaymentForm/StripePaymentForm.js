...
const LocationOrShippingDetails = props => {
  ...
  const { rentPickupLocation: { selectedPlace: { address: rentPickupAddress }={} } = {}, rentShippingLocation: { selectedPlace: { address: rentShippingAddress }={} } = {}, pickupAddressSell, deliveryAddressSell,rentalpickupAddress,rentaldeliveryAddress,customShippingAddressSell } = listing?.attributes?.publicData || {};
  const{rentPickupCity,rentPickupState,rentPickupZipCode
}=rentalpickupAddress||{}
const{rentShippingCity,rentShippingState,rentShippingZipCode}=
rentaldeliveryAddress||{}
const{customSellShippingCity,customSellShippingState,customSellShippingZipCode}=customShippingAddressSell||{}
  console.log(listing?.attributes?.publicData, '&&& &&& => listing?.attributes?.publicData');
  
  const { sellPickupCity, sellPickupState, sellPickupZipCode } = pickupAddressSell || {};
  const { sellShippingCity, sellShippingState, sellShippingZipCode } = deliveryAddressSell || {}; 

  const { deliveryMethod, productType ,deliveryfee} = pageData?.orderData || {};

  const locationDetails = rentPickupAddress && deliveryMethod == 'pickup' &&
    productType == 'rent' ? `${rentPickupCity},${rentPickupState},${rentPickupZipCode}` : rentShippingAddress &&
      productType == 'rent' && deliveryMethod == 'delivery' ? `${rentShippingCity},${rentShippingState},${rentPickupZipCode}` :deliveryMethod == 'customDelivery'? `${customSellShippingCity},${customSellShippingState},${customSellShippingZipCode}`:
    productType == 'sell' && (deliveryMethod == 'pickup'|| !deliveryfee) ? `${sellPickupCity},${sellPickupState},${rentShippingZipCode}` :
      productType == 'sell' && (deliveryMethod == 'delivery'||deliveryfee) ? `${sellShippingCity}, ${sellShippingState}, ${sellShippingZipCode}`
...
):  rentPickupAddress && deliveryMethod == 'pickup' &&
    productType == 'rent' ? (
      <div className={css.locationWrapper}>
        <Heading as="h3" rootClassName={css.heading}>
          <FormattedMessage id="StripePaymentForm.pickuplocationDetailsTitle" />
        </Heading>
        <p className={css.locationDetails}>{locationDetails}</p>
      </div>) :
  customSellShippingCity &&
      productType == 'sell' && deliveryMethod == 'customDelivery' ? (
      <div className={css.locationWrapper}>
        <Heading as="h3" rootClassName={css.heading}>
          <FormattedMessage id="StripePaymentForm.ShippinglocationDetailsTitle" />
        </Heading>
        <p className={css.locationDetails}>{locationDetails}</p>
      </div>) :
      rentShippingAddress &&
      productType == 'rent' && deliveryMethod == 'delivery' ? (
      <div className={css.locationWrapper}>
        <Heading as="h3" rootClassName={css.heading}>
          <FormattedMessage id="StripePaymentForm.deliverylocationDetailsTitle" />
        </Heading>
        <p className={css.locationDetails}>{locationDetails}</p>
      </div>) :
  productType == 'sell' && (deliveryMethod == 'pickup'|| !deliveryfee) ? (
      <div className={css.locationWrapper}>
        <Heading as="h3" rootClassName={css.heading}>
  ...
  productType == 'sell' && (deliveryMethod == 'delivery'||deliveryfee) ? (
      <div className={css.locationWrapper}>
        <Heading as="h3" rootClassName={css.heading}>
          <FormattedMessage id="StripePaymentForm.ShippinglocationDetailsTitle" />
        </Heading>
