...
import { FieldTextInput, PrimaryButton, StripePaymentAddress, Form, H4 } from '../../../components';
...
class SubscriptionCardPaymentForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      initialState,
  ...
  handleSubmit = values => {
    ...
    const { currentUser, dispatch, onClose } = this.props;
    const ensuredCurrentUser = ensureCurrentUser(currentUser);
    const stripeCustomerData = ensuredCurrentUser.stripeCustomer;
    ...
    dispatch(createStripeSetupIntent())
      .then(setupIntent => {
        ...
        return dispatch(handleCardSetup(stripeParams));
      })
      .then(result => {
        ...
        return dispatch(savePaymentMethod(stripeCustomerData, newPaymentMethod));
      })
      .then(() => {
  ...
  handleRemovePaymentMethod = () => {
    dispatch(deletePaymentMethod()).then(() => {
      dispatch(stripeCustomer());
    });
  };
  ...
  paymentForm(formRenderProps) {
    ...
    const submitDisabled = invalid || cardInputNeedsAttention || isSubmitting;
    const hasCardError = this.state.error && !isSubmitting;
...
SubscriptionCardPaymentForm.defaultProps = {
  className: null,
  rootClassName: null,
  inProgress: false,
  onSubmit: null,
...
SubscriptionCardPaymentForm.propTypes = {
  formId: string,
  onSubmit: func,
...
const EnhancedPaymentMethodsForm = props => {
  ...
  const intl = useIntl();
  return <SubscriptionCardPaymentForm config={config} intl={intl} {...props} />;
};
