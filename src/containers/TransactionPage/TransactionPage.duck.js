...
import { deleteCalenderExceptionsApi, getTxDataMongoApi, transactionLineItems } from '../../util/api';
...
export const FETCH_TX_MONGO_DATA_REQUEST = 'app/TransactionPage/FETCH_TX_MONGO_DATA_REQUEST';
export const FETCH_TX_MONGO_DATA_SUCCESS = 'app/TransactionPage/FETCH_TX_MONGO_DATA_SUCCESS';
export const FETCH_TX_MONGO_DATA_ERROR = 'app/TransactionPage/FETCH_TX_MONGO_DATA_ERROR';
...
const initialState = {
  ...
  txMongoData: null,
  fetchTxMongoDataInProgress: false,
  fetchTxMongoDataError: null,
};
...
export default function transactionPageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case FETCH_TX_MONGO_DATA_REQUEST:
      return { ...state, fetchTxMongoDataInProgress: true, fetchTxMongoDataError: null };
    case FETCH_TX_MONGO_DATA_SUCCESS:
      return { ...state, fetchTxMongoDataInProgress: false, txMongoData: payload };
    case FETCH_TX_MONGO_DATA_ERROR:
      return { ...state, fetchTxMongoDataInProgress: false, fetchTxMongoDataError: payload };
...
export const fetchTxMongoDataRequest = () => ({ type: FETCH_TX_MONGO_DATA_REQUEST });
export const fetchTxMongoDataSuccess = txMongoData => ({
  type: FETCH_TX_MONGO_DATA_SUCCESS,
  payload: txMongoData,
});
export const fetchTxMongoDataError = error => ({
  type: FETCH_TX_MONGO_DATA_ERROR,
  error: true,
  payload: error,
});
...
export const fetchTransaction = (id, txRole, config) => (dispatch, getState, sdk) => {
  ...
  return sdk.transactions
    ...
    .then(async response => { 
      ...
      const isNeededfetchdataFromMongo = transaction?.attributes?.protectedData?.fetchdataFromMongo || false
      if(isNeededfetchdataFromMongo){
        console.log(transaction, " checking transaction") 
        //get mongo Data to showListing
        dispatch(fetchTxMongoDataRequest());
        try {
          const txMongoData = await getTxDataMongoApi({tx_id: id?.uuid}); 
          dispatch(fetchTxMongoDataSuccess(txMongoData));
        } catch (error) {
          console.error('Error fetching MongoDB data:', error);
          dispatch(fetchTxMongoDataError(storableError(error)));
        }
      }
...
export const makeTransition = (txId, transitionName, params) => (dispatch, getState, sdk) => {
  ...
  return sdk.transactions
    ...
    .then(response => {
      ...
      // Delete booking for extra items if the transition was successful 
      const exceptionIds = response?.data?.data?.attributes?.metadata?.exceptionId;
      if (
        Array.isArray(exceptionIds) &&
        ...
        deleteCalenderExceptionsApi({ exceptionIds: exceptionIds });
      } 
      return response;
    })
    .catch(e => {
