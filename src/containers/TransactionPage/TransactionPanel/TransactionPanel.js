...
export class TransactionPanelComponent extends Component {
  ...
  render() {
    ...
    const { metadata, processName } = transaction?.attributes || {};
    ...
    // Check is order shipped
    const isOrdershipped =
      transaction?.attributes?.lastTransition == 'transition/operator-mark-shipped';
    // Show Provider to download the label
    const showDownloadLabelForProvider =
      processName == 'default-purchase' &&
      protectedData &&
      protectedData?.shippingData &&
      protectedData?.shippingData?.labelUrl &&
      lastTransition == 'transition/create-shipping-label' &&
      isProvider;
    // Show customer to track the order
    const showTrackOrderForCustomer =
      processName == 'default-purchase' &&
      protectedData &&
      protectedData?.shippingData &&
      protectedData?.shippingData?.trackingUrlProvider &&
      isOrdershipped;
    return (
          ...
          <div className={css.asideDesktop}>
            <div className={css.stickySection}>
              <div className={css.detailCard}>
                ...
                {/* {showDownloadLabelForProvider ? (
                  <a
                    href={metadata?.trackingInfo?.labelUrl}
                    target="_blank"
                ...
                {/* Tranck Order for Customer  */}
                {/* {showTrackOrderForCustomer ? (
                  <a
                    href={metadata?.trackingInfo?.trackingUrlProvider}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                ...
                {showDownloadLabelForProvider ? (
                  <div className={css.labelBox}>
                    <a
                      className={css.downloadLabelButton}
                      href={protectedData?.shippingData?.labelUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      Download label
                ...
                {showTrackOrderForCustomer ? (
                  <div className={css.labelBox}>
                    <a
                      className={css.downloadLabelButton}
                      href={protectedData?.shippingData?.trackingUrlProvider}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      Track Your Order
                ...
                {/* <div className={css.detailCardContent}>
                {protectedData && protectedData.securityDeposit ? (
                  <div>
                    Security Deposit amount - ${protectedData.securityDeposit.amount / 100}
                  </div>
                ) : null}
                {protectedData?.additionalfeeRequested?.length > 0 && (
                  <div>
                    Damaged Claim amount - ${protectedData.additionalfeeRequested[0].AdditionalPrice / 100}
                  </div>
                )}
                </div> */}
