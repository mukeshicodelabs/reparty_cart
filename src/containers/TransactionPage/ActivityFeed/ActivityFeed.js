...
const TransitionMessage = props => {
  ...
  if (transition.transition == 'transition/agreement-sign-initiation' && ownRole === TX_TRANSITION_ACTOR_CUSTOMER) {
    return "The provider has signed. Please check your email from HelloSign to complete your signature";
  }
  if (transition.transition == 'transition/agreement-sign-by-customer' && ownRole === TX_TRANSITION_ACTOR_PROVIDER) {
    return "The customer has signed.";
  }
  else{
    return message;
  }
...
export const ActivityFeed = props => {
  ...
  const images = transaction.attributes?.protectedData?.mediaOrderReceive;
    ...
    return (
      <>
        ...
        <li key={transition.transition} className={css.transitionItem}>
          {transition.transition === 'transition/mark-order-receive' && images && Array.isArray(images) && images?.length && images?.map(image => {

            return (
              <li className={css.txLinks}>
                <div className={css.imageUrl}>
                  <a className={css.returnImage} href={image?.file?.url} target='blank'>
                    <img src={image?.file?.url} alt="Uploaded" />
                  </a>
                </div>
              </li>
            )
