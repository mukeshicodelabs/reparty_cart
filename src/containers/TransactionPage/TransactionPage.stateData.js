...
export const getStateData = (params, process) => {
    ...
    getActionButtonPropsMaybe(
      {
        ...
        transitionError,
        onAction: (data) => onTransition(transaction?.id, transitionName, data?.protectedData ? {protectedData: data.protectedData} : {}),
        ...extra,
      },
      forRole
    );
  ...
  if (processName === PURCHASE_PROCESS_NAME || processName === PURCHASE_PROCESS_NAME_CART) {
