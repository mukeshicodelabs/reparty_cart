...
import { onCalculateDistance, transactionLineItems } from '../../util/api';
...
export const FETCH_LISTINGS_REQUEST = 'app/ListingPage/FETCH_LISTINGS_REQUEST';
export const FETCH_LISTINGS_SUCCESS = 'app/ListingPage/FETCH_LISTINGS_SUCCESS';
export const FETCH_LISTINGS_ERROR = 'app/ListingPage/FETCH_LISTINGS_ERROR';

export const ADD_OWN_ENTITIES = 'app/ListingPage/ADD_OWN_ENTITIES';

export const CALCULATE_LOCATION_REQUEST = 'app/ListingPage/CALCULATE_LOCATION_REQUEST';
export const CALCULATE_LOCATION_SUCCESS = 'app/ListingPage/CALCULATE_LOCATION_SUCCESS';
export const CALCULATE_DISTANCE_ERROR = 'CALCULATE_DISTANCE_ERROR';
...
const initialState = {
  ...
  calculateLocationRequest:false,
  calculatedDistance:null,
  calculateLocationError:false,
};
...
const listingPageReducer = (state = initialState, action = {}) => {
  ...
  switch (type) {
    ...
    case ADD_OWN_ENTITIES:
     ...
     case CALCULATE_LOCATION_REQUEST:
      return { ...state, calculateLocationRequest: true };
    case CALCULATE_LOCATION_SUCCESS:
      return { ...state, calculateLocationRequest: false,calculatedDistance:payload };
    case CALCULATE_DISTANCE_ERROR:
      return { ...state, calculateLocationError: true,calculateLocationRequest: false};  
...
export const calculateLocationRequest=()=>({ type:CALCULATE_LOCATION_REQUEST })
export const calculateLocationSuccess=(data)=>({ type:CALCULATE_LOCATION_SUCCESS, payload:data})
export const calculateDistanceError = (error) => ({
  type: CALCULATE_DISTANCE_ERROR,
  payload: error
});
...
export const calculateDistance = (params) => async (dispatch, getState, sdk) => {
  const { listingId, userLocation } = params;

  dispatch(calculateLocationRequest());
  try {
    ...
    const customSellShippingLocation=listing?.attributes?.publicData?.customSellShippingLocation;
    const { productType } = listing?.attributes?.publicData || {};
    const shippingLocation = productType == 'rent' ?
      listing?.attributes?.publicData?.rentShippingLocation : customSellShippingLocation;

    if (!shippingLocation || !userLocation) {
      throw new Error('Missing location data for distance calculation');
    }

    const distanceData = await onCalculateDistance({
      shippingLocation,
      userLocation,
      listing
    });

    dispatch(calculateLocationSuccess(distanceData));
    return distanceData;
  } catch (e) {
    console.error('Error in calculateDistance:', e);
    dispatch(calculateDistanceError(e));
    throw e;
  }
};

export const queryAuthorListings = queryParams => (dispatch, getState, sdk) => {
...
export const loadData = (params, search, config) => async (dispatch, getState, sdk) => {
  ...
  // Get listing and authorId first
  const listingResponse = await dispatch(showListing(listingId, config));
  const listing = listingResponse?.data?.data;
  const authorId = listing?.relationships?.author?.data?.id?.uuid ?? null;

  if (!authorId) {
    console.warn(`No authorId found for listing ${listingId.uuid}`);
  }


  // Prepare image query params
  const page = 1;
  ...
  // Construct base queryParams including authorId
  const queryParams = {
    authorId, // ✅ inject authorId here
  };

  const ownListingsPromise = authorId ?
    dispatch(
      queryAuthorListings({
        ...queryParams,
        page,
  ...
  const promises = hasNoViewingRights
    ? [ownListingsPromise]
    : [dispatch(fetchReviews(listingId)), ownListingsPromise];
  ...
  return {
    listingResponse,
    reviewsResponse: hasNoViewingRights ? null : responses[0],
    ownListingsResponse: responses[hasNoViewingRights ? 0 : 1],
  };
};
