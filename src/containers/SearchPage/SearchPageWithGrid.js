...
import { H3, H5, NamedRedirect, Page, PrimaryButton } from '../../components';
...
export class SearchPageComponent extends Component {
  ...
  handleResetLocation() {
    const { history, routeConfiguration, location } = this.props;
    const urlQueryParams = validUrlQueryParamsFromProps(this.props);
    console.log('handleResetLocation', urlQueryParams);
    const { address, bounds, ...rest } = urlQueryParams || {};
    this.setState({ currentQueryParams: { ...rest } });
    const { routeName, pathParams } = getSearchPageResourceLocatorStringParams(
      routeConfiguration,
      location
    );
    history.push(
      createResourceLocatorString(routeName, routeConfiguration, pathParams, { ...rest })
    );
  }
  ...
  render() {
    ...
    const hasAddress = search && new URLSearchParams(search).has('address');    
    ...
    .map(key => availableFilters.find(filter => filter.key === key))
    ...
    return (
      ...
      >
        ...
        <div className={css.topSearchBar}>
          <div className={css.searchWrapper}>
            <h3 className={css.searchPageheading}>
            ...
            <div className={css.searchContainer}>
              ...
              {hasAddress ? (
                ...
                >
                  <BrandIconCard type="close" />
                  {/* <FormattedMessage id="SearchPage.locationsearhResetButton" /> */}
                </PrimaryButton>
              ) : null}
            </div>
          </div>
            ...
            <aside className={css.layoutWrapperFilterColumn} data-testid="filterColumnAside">
              <div className={css.filterColumnContent}>
                <div className={css.filterHeading}>
                  <BrandIconCard type="filters" />
                  <FormattedMessage id="SearchFilterDesktop.filtersTitle" />
                </div>
                {orderedFilters.map(filterConfig => {
...
const EnhancedSearchPage = props => {
  ...
  const { currentUser, onUpdateProfile, ...restOfProps } = props;
