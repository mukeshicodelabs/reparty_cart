...
import seller from '../../assets/images/comunity-icon/founder-seller.svg'
import party from '../../assets/images/comunity-icon/party-pro.svg'
import topSellerIcon from '../../assets/images/comunity-icon/top-seller.svg'
import comunity from '../../assets/images/comunity-icon/comunity-fav.svg'
import friendly from '../../assets/images/comunity-icon/eco-friendly.svg'
import great from '../../assets/images/comunity-icon/great.svg'
import photo from '../../assets/images/comunity-icon/photo.svg'
import superHost from '../../assets/images/comunity-icon/super-host.svg'
import repartyInsiderIcon from '../../assets/images/comunity-icon/insider.svg'
...
export const MainContent = props => {
  ...
  const comunitylist = [
    { key: 'foundingSeller', icon: seller, title: 'Founding Seller' },
    { key: 'partyPro', icon: party, title: 'Party Pro' },
    { key: 'topSeller', icon: topSellerIcon, title: 'Top Seller' },
    { key: 'communityFavorite', icon: comunity, title: 'Community Favorite' },
    { key: 'ecoFriendlyseller', icon: friendly, title: 'Eco-Friendly Seller' },
    { key: 'greatCommunicator', icon: great, title: 'Great Communicator' },
    { key: 'photoReadyListing', icon: photo, title: 'Photo-Ready Listings' },
    { key: 'superhost', icon: superHost, title: 'Super Host' },
    { key: 'repartyInsider', icon: repartyInsiderIcon, title: 'ReParty Insider' },
  ];
  const activeItem = comunitylist.find(item => currentUser?.attributes?.profile?.publicData?.[item.key]);
  ...
  return (
    <div>
      <div className={css.topAvatarWrapper}>
      <AvatarLarge className={css.avatar} user={user} disableProfileLink />
      <H2 as="h1" className={css.desktopHeading}>
        <FormattedMessage id="ProfilePage.desktopHeading" values={{ name: displayName }} />
      </H2>
       <Button className={css.shareButton} onClick={() => setIsShareModalOpen(true)}>
        <IconCollection icon="share" />
        Share
      </Button>
      </div>
      
      <div className={css.comunityBox}>
        <div className={css.comunityBoxWrapper}>
          {(currentUser?.id?.uuid === listings[0]?.author?.id?.uuid
            ? comunitylist.filter(item => currentUser?.attributes?.profile?.publicData?.[item.key])
            : comunitylist.filter(item => listings[0]?.author?.attributes?.profile?.publicData?.[item.key])
          )?.map(activeItem => (
            <div key={activeItem.key} className={css.comunityCard}>
              <img src={activeItem.icon} alt={activeItem.title} />
              <div className={css.comunityHeading}>{activeItem.title}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <Modal
        id="ShareModal"
        containerClassName={css.shareModal}
        isOpen={isShareModalOpen}
        onClose={() => setIsShareModalOpen(false)}
        usePortal
        onManageDisableScrolling={() => { }}
        closeButtonMessage={intl.formatMessage({ id: 'ManageListingCard.closeModal' })}
      >
        <SocialShare
        userID={user?.id?.uuid}
        />
...
export const ProfilePageComponent = props => {
  ...
  return (
    ...
    >
      ...
      >
        <div className={css.profileMainContent}>
          <MainContent
            bio={bio}
            displayName={displayName}
