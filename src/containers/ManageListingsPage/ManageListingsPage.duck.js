...
const getListingParam = tab => {
  const tabMap = {
    active: { states: 'published' },
    draft: { states: 'draft' },
    closed: { states: 'closed' },
    pendingReviews: { states: 'pendingApproval' },
    declined: { pub_declined: true },
  };
...
export const discardDraft = listingId => (dispatch, getState, sdk) => {
  ...
  return sdk.ownListings
    ...
    .then(([_, listingResponse]) => {  
      const filteredListings= listingResponse?.data?.data?.filter(
        listing => listing.id.uuid !== listingId?.uuid) 
        const newResponse = {
          ...listingResponse,  
          data: { ...listingResponse.data, data: filteredListings }  
        };
      dispatch(addOwnEntities(newResponse));
      dispatch(queryListingsSuccess(newResponse));
...
export const loadData = (params, search, config) => (dispatch, getState, sdk) => {
  ...
  const paramTab = getListingParam(params.tab);
