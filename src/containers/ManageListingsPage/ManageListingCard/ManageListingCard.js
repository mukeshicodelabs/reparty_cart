...
export const ManageListingCard = props => {
  ...
  const { isAiListing, isManualListing } = publicData || {};
  const slug = createSlug(title);
  const isPendingApproval = state === LISTING_STATE_PENDING_APPROVAL;
  const isClosed = state === LISTING_STATE_CLOSED;
  const isDraft = state === LISTING_STATE_DRAFT;

  const { listingType, transactionProcessAlias, productType, heroImageId } = publicData || {}; 
  const listingProductType = allProductsType?.find(value => value.key === productType)?.label;
 ...
 const pintrestImage =  firstImage?.attributes?.variants?.['listing-card']?.url || '';
  return (
    <div className={classes}>
      ...
      <div className={css.info}>
        ...
        <div className={css.manageLinks}>
          ...
          <Button className={css.searchButton} onClick={() => setIsShareModalOpen(true)}>
