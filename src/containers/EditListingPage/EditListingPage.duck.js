...
import { types as sdkTypes, createImageVariantConfig, createResponsiveImageVariants } from '../../util/sdkLoader';
...
import { createAiListing, createSearchDataApi, generateAiImage, getdetailsfromApi, updateSearchDataApi, zendeskMail } from '../../util/api';

import { categorLabel } from '../../util/constants';
import { getCategoryLabel, getColorLabels, getEventTypeLabels } from '../../util/helper';
...
//   };
// };
const getImageVariantInfo = (listingImageConfig, actualImageDimensions = null) => {
  const { variantPrefix = 'listing-card' } = listingImageConfig;

  // Use the new responsive image variants function
  const imageVariants = createResponsiveImageVariants(variantPrefix, actualImageDimensions);
...
export const FETCH_AI_DATA_REQUEST = 'app/EditListingPage/FETCH_AI_DATA_REQUEST';
export const FETCH_AI_DATA_SUCCESS = 'app/EditListingPage/FETCH_AI_DATA_SUCCESS';
export const FETCH_AI_DATA_ERROR = 'app/EditListingPage/FETCH_AI_DATA_ERROR';

export const FETCH_AI_IMAGE_REQUEST = 'app/EditListingPage/FETCH_AI_IMAGE_REQUEST';
export const FETCH_AI_IMAGE_SUCCESS = 'app/EditListingPage/FETCH_AI_IMAGE_SUCCESS';
export const FETCH_AI_IMAGE_ERROR = 'app/EditListingPage/FETCH_AI_IMAGE_ERROR';
...
const initialState = {
  ...
  aiData: {},
  aiDataError: null,
  aiDataInProgress: false,

  aiImage: {},
  aiImageError: null,
  aiImageInProgress: false,
};
...
export default function reducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case UPLOAD_IMAGE_SUCCESS: {
      ...
      return { ...state, uploadedImages, uploadImageInProgress: false };
    }
    ...
    case FETCH_AI_DATA_REQUEST:
      return { ...state, aiDataInProgress: true, aiDataError: null, aiData: {} };
    case FETCH_AI_DATA_ERROR:
      return { ...state, aiDataInProgress: false, aiDataError: payload.error };
    case FETCH_AI_DATA_SUCCESS:
      return { ...state, aiDataInProgress: false, aiData: payload };

    case FETCH_AI_IMAGE_REQUEST:
      return { ...state, aiImageInProgress: true, aiImageError: null, aiData: {} };
    case FETCH_AI_IMAGE_ERROR:
      return { ...state, aiImageInProgress: false, aiImageError: payload.error };
    case FETCH_AI_IMAGE_SUCCESS:
      return { ...state, aiImageInProgress: false, aiImage: payload };
...
export const fetchAiModalDataRequest = () => ({
  type: FETCH_AI_DATA_REQUEST,
});
export const fetchAiModalDataSuccess = response => {
  return {
    type: FETCH_AI_DATA_SUCCESS,
    payload: response.data,
  };
};
export const fetchAiModalDataError = error => ({
  type: FETCH_AI_DATA_ERROR,
  payload: { error },
  error: true,
});

export const fetchAiImageRequest = () => ({
  type: FETCH_AI_IMAGE_REQUEST,
});
export const fetchAiImageSuccess = response => {
  return {
    type: FETCH_AI_IMAGE_SUCCESS,
    payload: response.data,
  };
};
export const fetchAiImageError = error => ({
  type: FETCH_AI_IMAGE_ERROR,
  payload: { error },
  error: true,
});
...
export async function fetchImageDecs(data, dispatch) {
  try {
    const resp = await getdetailsfromApi({ metadata: data });
    if (resp?.status === 'success') {
      return resp?.data || {};
    }
  } catch (error) {
    // dispatch(showListingsError(storableError(error)));
  }
}
...
const makeAlgoliaImageURLS = image => {
  const imageURLS = [];
  const allVariants = image?.attributes?.variants;
  if (allVariants && Object.keys(allVariants).length) {
    Object.keys(allVariants).map(st => {
      imageURLS.push({
        ...allVariants[st],
        variantType: st,
      });
...
export const updateAlgoliaSearchData = (data, config) => (dispatch, getState, sdk) => {
  
  const listingFields = config?.listing?.listingFields || [];
  const { id, attributes, currentStock } = data?.data;
  const { title, price, description, state, publicData, createdAt } = attributes;
  const {
    category,
    listingState,
    event_type,
  ...
  const itemStock = data?.included[2]?.attributes?.quantity || 0;

  const UpdatedCategoryLabel = getCategoryLabel(category);
  const updatedEventType = getEventTypeLabels(event_type, config);
  const updatedColorLabel = getColorLabels(select_color, config);

  const authorId = currentUser?.id?.uuid || '';
  const { profileImage } = currentUser || {};
  const images = Array.isArray(data?.included)
    ? data?.included.filter(st => st.type == 'image')
    : [];
  const imageId = images.length && images[0].id.uuid;
  const allImages = images.filter(st =>
    heroImageId ? st.id.uuid == heroImageId : st.id.uuid == imageId
  );
  const imageURLS = makeAlgoliaImageURLS(allImages.length ? allImages[0] : null);
  const listingImageUrl =
    Array.isArray(allImages) && allImages.length
      ? allImages[0]?.attributes?.variants?.['scaled-large']?.url
      : '';
  const listing = {
    objectID: id?.uuid,
    state,
    ...
    description,
    listingState,
    category: UpdatedCategoryLabel,
    event_type: updatedEventType,
    productType: productType == 'rent' ? 'Rent' : 'Sell',
    select_color: updatedColorLabel,
    selectedCategoryOptions,
    pickupFullAddress: pickupFullAddress,
    price: price?.amount || 0,
    stock: productType == 'rent' ? parseInt(rentStock) : itemStock,
    listingImageUrl,
    imageURLS,
    authorData: {
  ...
  return (dispatch, getState, sdk) => {
    ...
    return updateStockOfListingMaybe(id, stockUpdate, dispatch)
      ...
      .then(response => {
        ...
        dispatch(updateAlgoliaSearchData(response?.data, config));
...
export const createAlgoliaSearchData = (data, config) => (dispatch, getState, sdk) => {
  const { id, attributes, currentStock } = data?.data;
  
 
  const { title, price, description, state, publicData, geolocation,createdAt } = attributes;
  const {
    category,
    event_type,
    productType,
  ...
  const authorId = currentUser?.id?.uuid || '';
  const { profileImage = {} } = currentUser || {};
  const itemStock = data?.included?.find(item => item.type === "stock")?.attributes?.quantity || 0;
  const UpdatedCategoryLabel = getCategoryLabel(category);
  const updatedEventType = getEventTypeLabels(event_type, config);
  const updatedColorLabel = getColorLabels(select_color, config);
...
const imageVariantInfo = getImageVariantInfo(listingImageConfig);
  return sdk.ownListings
    ...
    .then(response => { 
      ...
      const isPublished =   attributes?.state == "published" || false;
          ...
          : dispatch(
              uploadImageSuccess({
                data: {
                  ...
                  isAiImage: actionPayload?.isAiImage,
                },
              })
            );
        return img;
...
export const updateNumberOfAiImagesCreation = (id, count = 0) => (dispatch, getState, sdk) => {
  sdk.ownListings
    .update(
      {
        id: id,
        publicData: {
          numberOfImagesGenterated: count || 0,
        },
      },
      {
        expand: true,
    ...
    .then(res => {
      // console.log('Lisint has been updated &&& &&& => res');
    });
};
...
export const getAiListingCreationData = (params, heroImageId) => (dispatch, getState, sdk) => {
  dispatch(fetchAiModalDataRequest());
  return createAiListing({
    images: params.images,
    heroImageId: heroImageId,
  })
    .then(response => {
      dispatch(fetchAiModalDataSuccess(response));
      return response;
    })
    .catch(err => {
      console.error(err, '---- getVendorCredits ---- => err');
      dispatch(fetchAiModalDataError(storableError(err)));
    });
};
export const getAiListingImage = params => (dispatch, getState, sdk) => {
  dispatch(fetchAiImageRequest());
  return generateAiImage({
    images: params.images,
    description: params.description,
  })
    .then(response => {
      dispatch(fetchAiImageSuccess(response));
      return response;
    })
    .catch(err => {
      console.error(err, '---- getVendorCredits ---- => err');
      dispatch(fetchAiImageError(storableError(err)));
    });
};
