...
import {
  ...
  removeListingImage,
  savePayoutDetails,
  getAiListingCreationData,
  getAiListingImage,
  fetchImageDecs,
  updateNumberOfAiImagesCreation,
...
const pickRenderableImages = (
  ...
  const unattachedImages = uploadedImageIdsInOrder.map(i => uploadedImages[i]).filter(img => !!img);;
...
const mapStateToProps = state => {
  ...
  const {uploadImageInProgress} = state.EditListingPage;
  const {
    getAccountLinkInProgress,
    getAccountLinkError,
...
const mapDispatchToProps = dispatch => ({
  ...
  onGetAiListingCreationData: (data, heroImageId) => dispatch(getAiListingCreationData(data, heroImageId)),
  onGetAiListingImage: params => dispatch(getAiListingImage(params)),
  onfetchImageDesc: metadata => dispatch(fetchImageDecs(metadata)),
  onupdateNumberOfAiImagesCreation: (id, numberOfAiImages) =>dispatch(updateNumberOfAiImagesCreation(id, numberOfAiImages)),
