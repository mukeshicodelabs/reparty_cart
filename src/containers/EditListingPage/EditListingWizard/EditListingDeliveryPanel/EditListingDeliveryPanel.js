...
const getInitialValues = props => {
  ...
  const locationFieldsPresent = publicData?.pickuplocation?.address && geolocation;
  const location = publicData?.pickuplocation || {};
 ...
 const { address } = location;
  ...
  const depositFeeAsMoney = depositFee ? new Money(depositFee?.amount, depositFee.currency) : null;
  const lateFeeAsMoney = lateFee ? new Money(lateFee?.amount, lateFee.currency) : null;
  const pickupFeeAsMoney = pickupFee ? new Money(pickupFee?.amount, pickupFee.currency) : null;
  const setupPriceAsMoney = setupPrice ? new Money(setupPrice?.amount, setupPrice.currency) : null;
  const deliveryFeeAsMoney = deliveryFee ? (deliveryFee || []).reduce((acc, item) => {
    const { range, rangeFee } = item || {};

    acc[range] = {
      rangeFee: new Money(rangeFee?.amount, rangeFee?.currency),
    };
    return acc;
  }, {}) : {};
      ...
      {priceCurrencyValid ? (
        <EditListingDeliveryForm
          ...
          onSubmit={values => {
            ...
            const address = pickuplocation?.selectedPlace?.address || null;
            const origin = pickuplocation?.selectedPlace?.origin || null;
            const formattedDelivery = Object.entries(delivery || {}).map(([key, value]) => ({
              range: key,
              rangeFee: { amount: value?.rangeFee?.amount, currency: value?.rangeFee?.currency },
            }));
            const pickupDataMaybe =
              pickupEnabled && address ? { pickuplocation: { address, building } } : {};
            ...
            const updateValues = {
              ...
              publicData: {
                ...
                packageHeight,
                packageWeight,
                // pickuplocation: pickuplocation?.selectedPlace ? { address: address } : null,
                building,
                deliveryFee: formattedDelivery,
                zipCode,
                pickupFee: pickupFee ? { amount: pickupFee?.amount, currency: pickupFee?.currency } : null,
                setupPrice: setupPrice ? { amount: setupPrice?.amount, currency: setupPrice?.currency } : null,
                lateFee: lateFee ? { amount: lateFee?.amount, currency: lateFee?.currency } : null,
                depositFee: depositFee ? { amount: depositFee?.amount, currency: depositFee?.currency } : null
              },
            };

            const deliveryFeeAsMoney = formattedDelivery ? (formattedDelivery || []).reduce((acc, item) => {
              const { range, rangeFee } = item || {};
            ...
            setState({
              initialValues: {
                building,
                pickuplocation: { search: address, selectedPlace: { address, origin } },
                deliveryOptions,
                customAddOns,
                packageWidth,
                packageLength,
