...
import { getCategoryKeyByLabel } from '../../../../util/helper';
...
const EditListingDetailsPanel = props => {
  ...
  const { email, profile: { protectedData: { phoneNumber } = {} } = {} } =
    currentUser?.attributes || {};
  const [uploadedVideo, setUploadedVideo] = React.useState(
    listing?.attributes?.publicData?.uploadedVideoURL || ''
  );
  const [usedAiFortitle, setUsedAiFortitle] = React.useState(
    listing?.attributes?.publicData?.usedAiFortitle || 0
  );
  ...
  // Select the hero image ID from the listing's public data
  const [heroImageId, setHeroImageId] = useStateRef('');
  useEffect(() => {
    setHeroImageId(listing?.attributes?.publicData?.heroImageId);
  }, []);
    ...
    <div className={classes}>
      {canShowEditListingDetailsForm ? (
        <EditListingDetailsForm
          setUsedAiFortitle={setUsedAiFortitle}
          usedAiFortitle={usedAiFortitle}
          ...
          onSubmit={values => {
            const { title, description, images, category, aiTags } = values;

            const categorykey = getCategoryKeyByLabel(category, categories);
            const originalCategory = listing?.attributes?.publicData?.category;
            const categoryChanged = categorykey !== originalCategory;
            const selectedCategoryOptions =
              listing?.attributes?.publicData?.selectedCategoryOptions;

            // New values for listing attributes
            const updateValues = {
              title: title || 'dummy',
              description: description || listing?.attributes?.publicData?.description || 'dummy',
              images: images,
              publicData: {
                usedAiFortitle: usedAiFortitle || listing?.attributes?.publicData?.usedAiFortitle,
                heroImageId: heroImageId
                  ? heroImageId
                  : listing?.attributes?.publicData?.heroImageId,
                aiTags: aiTags || listing?.attributes?.publicData?.aiTags,
                uploadedVideoURL: uploadedVideo,
                category: categorykey || listing?.attributes?.publicData?.category,
                selectedCategoryOptions:
                  categoryChanged && categorykey != null ? null : selectedCategoryOptions,
                autherEmail: email ? email : null,
                autherPhoneNumber: phoneNumber ? phoneNumber : null,
                isFeatured:false,
                featureListingOrder:0
              },
            };
