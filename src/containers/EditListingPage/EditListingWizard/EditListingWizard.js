...
const TABS_AIDRIVEN = [DETAILS, ADDITIONAL_FORM, PARTY_THEME, PRICING_AND_STOCK, PHOTOS];
...
const tabLabelAndSubmit = (intl, tab, isNewListingFlow, isPriceDisabled, processName) => {
  ...
  } else if (tab === ADDITIONAL_FORM) {
    labelKey = 'EditListingWizard.tabLabelPartyTheme';
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.additonalForm`;
  } else if (tab === PARTY_THEME) {
    labelKey = 'EditListingWizard.tabLabelPartyTheme';
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.savepartyTheme`;
...
class EditListingWizard extends Component {
  ...
  handlePublishListing(id) {
    ...
    if (
      ...
      onPublishListingDraft(id, config);
    } else {
      this.setState({
        draftId: id,
  ...
  render() {
    ...
    const isAiRedirection = params.custom == 'ai';
    ...
    //  const tabs = isAiRedirection ? TABS_AIDRIVEN : TABS_MANUAL;
    const tabs = TABS_AIDRIVEN
    ...
    const formattedMarketplaceName =
      marketplaceName.toLowerCase() === 'reparty' ? 'ReParty' : marketplaceName;

    const payoutModalInfo = stripeAccountData ? (
      <FormattedMessage id="EditListingWizard.payoutModalInfo" values={{ marketplaceName: formattedMarketplaceName }} />
    ) : (
      <FormattedMessage id="EditListingWizard.payoutModalInfoNew" values={{ marketplaceName: formattedMarketplaceName }} />
    );
        ...
        >
          <div className={css.modalPayoutDetailsWrapper}>
            ...
            {!currentUserLoaded ? (
              ...
              <>
                <div className={css.payoutInfo}>
                  <div className={css.payoutHeading}>One More Thing — Stripe Payout Setup</div>
                  <p className={css.payoutDescription}>
                    We use <span className={css.boldText}>Stripe</span> — the same secure payment partner trusted by <span className={css.boldText}>Airbnb, Lyft, Shopify, and millions of businesses worldwide</span> — to send your payouts.
                  </p>
                  <p className={css.payoutDescription}><span className={css.boldText}>All information is collected and stored by Stripe — ReParty never sees your SSN, bank details, or ID.</span></p>

                  <div className={css.mainView}>
                    <div className={css.listHeading}>Have These Ready</div>
                    <ul className={css.listView}>
                      <li><span className={css.boldText}>Full legal name</span> (exactly as on your ID)</li>
                      <li><span className={css.boldText}>Date of birth</span></li>
                      <li><span className={css.boldText}>Last 4 digits of your SSN</span></li>
                      <li><span className={css.boldText}>Current home address</span> (matches public records)</li>
                      <li><span className={css.boldText}>Bank routing & account number</span></li>
                      <li><span className={css.boldText}>Photo ID</span> (only if Stripe requests it)</li>
                    </ul>
                  </div>
                  <div className={css.mainView}>
                    <div className={css.listHeading}>Tips for Faster Approval</div>
                    <ul className={css.listView}>
                      <li>Match your ID name exactly — no nicknames</li>
                      <li>Include your full address, apartment/unit # if applicable</li>
                      <li>If you’ve moved recently, try your previous address first</li>
                    </ul>
                  </div>
                  <div className={css.noteText}>
                    <span className={css.boldText}>Note:</span> Stripe may ask for your full SSN or photo ID if records don’t match — this is normal, secure, and only happens once!
                  </div>
                </div>
